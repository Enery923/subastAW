...
function procesaFecha() {
	...
	if (peticionCierre.readyState == 4 && peticionCierre.status == 200) {
		...
		a = json.finSegundo.split(/[- :.]/);
		finSegundo = new Date(a[0], a[1]-1,a[2],a[3],a[4],a[5]);
		tiempoCierreSegundo = finSegundo.getTime() - ahora.getTime();
		setTimeout(update, tiempoCierre + 1000);
		setTimeout(update, tiempoCierreSegundo + 1000);
	}
}
